...
The `pyth-oracle-v4` contract is exposing the following method:
...
- introduced a utility function `set-price-testnet` in the `pyth-storage-v4` contract to set the price data for a specific feed without the need to produce a valid attestation. This function will only work on testnet.
