...
export namespace wormhole {
  ...
  export namespace fc_ext {
    ...
    export const vaaHeader = (
      ...
      let specifiedSignatures = fc.array(
  ...
  export const buildValidVaaHeaderSpecs = (
    ...
    for (let guardian of keychain) {
      ...
      if (signature.recovery) {
        ...
        signatures.push(
          Buffer.concat([id, signature.toCompactRawBytes(), rec]),
        );
      } else {
        const rec = Buffer.alloc(1);
        rec.writeUint8(0, 0);
  ...
  export const validGuardianRotationModule = Buffer.from(
  ...
  export function applyGuardianSetUpdate(
  ...
  export function applyMainnetGuardianSetUpdates(
    txSenderAddress: string,
    contractName: string,
  ) {
